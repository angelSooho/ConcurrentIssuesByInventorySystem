...
class OptimisticLockStockFacadeTest extends IntegrationTestSupport {
    ...
    @DisplayName("상품을 동시에 100명이 주문하면, 낙관락에 의해 동시성 문제가 발생하지 않고 재고가 0이 된다.")
    @Test
    public void sameTimeOrderTest() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try {
                    optimisticLockStockFacade.decrease(savedStock.getId(), 1L);
